...
namespace BoA.Solutions.Weather.Web.Api.UnitTests.Clients.AccuWeatherClient
{
	...
	public class when_finding_city_by_text
	{
		[Theory]
		[InlineData(null)]
		[InlineData("")]
		[InlineData(" ")]
		[InlineData("\n")]
		public async Task throw_ArgumentException_when_SearchText_is_IsNullOrWhiteSpace(string searchText)
		{
			...
			var ex = await Assert.ThrowsAsync<ArgumentException>(() => client.FindCityAsync(searchText));

			Assert.Equal("searchText", ex.ParamName);
		}
	}
}
